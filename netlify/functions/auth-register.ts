...
export const handler: Handler = async (event) => {
  ...
  try {
    const { email, password, name, country, referralCode } = JSON.parse(event.body || '{}');
    ...
    if (preExistingUser) {
      ...
      if (authUser) {
        ...
        if (!existingProf) {
          ...
          console.log(`[Netlify Auth Function] Profile was already auto-created by database trigger for: ${cleanEmail}`);
        }
      }
    }
