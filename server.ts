...
async function startServer() {
  ...
  app.post('/api/auth/register', async (req, res) => {
    const { email, password, name, country, referralCode } = req.body;
      ...
      if (preExistingUser) {
        ...
        if (authUser) {
          ...
          if (!existingProf) {
            ...
            console.log(`[Admin Auth Backend] [STEP 4/5] Profile was already auto-created by database trigger for: ${cleanEmail}`);
          }
        }
      }
  ...
  app.post(['/api/chariow/pulse', '/api/chariow/webhook'], async (req, res) => {
    console.log('[Chariow Webhook] Pulse received:', JSON.stringify(req.body, null, 2));
    ...
    try {
      ...
      if (isRejectedEvent) {
        ...
        console.log(`[Chariow Webhook] Vente non finalisée (${eventType}). Commission ${mostRecentPending.id} passée en 'rejected'.`);
        return res.status(200).json({
          success: true,
        ...
        console.log(`[Chariow Webhook] Vente finalisée (${eventType}). Commission ${mostRecentPending.id} passée en 'finalized'.`);
        return res.status(200).json({
          success: true,
