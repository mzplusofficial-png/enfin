...
export const AffiliationSystem: React.FC<AffiliationSystemProps> = ({ 
  ...
  if (isAdminView && showValidations) {
    const pending = commissions.filter(c => c.status === 'pending' || c.status === 'finalized');
    const history = commissions.filter(c => c.status === 'approved' || c.status === 'rejected');
                  ...
                  <div className="pt-3 border-t border-white/5 flex flex-col gap-3">
                    <div className="flex justify-between items-center text-[8px] text-neutral-600 font-mono">
                      ...
                      <span className={`px-2 py-1 rounded border font-black text-[7px] ${
                        ...
                        {c.status === 'finalized' ? '⚡ Vente Finalisée (Paid Chariow)' : '⏳ Clic/Init (En attente)'}
                      </span>
                    </div>
                    <div className="flex gap-2">
                      <button 
                        disabled={true}
                        className="flex-1 py-1.5 bg-neutral-800 text-neutral-500 disabled:opacity-50 font-black text-[8px] uppercase tracking-wider rounded-lg transition-all cursor-not-allowed flex items-center justify-center gap-1"
                      >
                        🔒 Approuver (Désactivé)
                      </button>
                      <button 
                        disabled={true}
                        className="flex-1 py-1.5 bg-neutral-900 text-neutral-600 border border-neutral-800 disabled:opacity-50 font-black text-[8px] uppercase tracking-wider rounded-lg transition-all cursor-not-allowed flex items-center justify-center gap-1"
                      >
                        🔒 Refuser (Désactivé)
                      </button>
                    </div>
             ...
             <table className="w-full text-left text-xs">
                <thead className="bg-black/40 text-[9px] font-black uppercase text-neutral-500 border-b border-neutral-800">
                  <tr>
                    ...
                    <th className="p-6">Statut d'achat</th>
                      ...
                      <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="p-6 text-right">
                          <div className="flex justify-end gap-2">
                            <button 
                              disabled={true}
                              className="px-3 py-1.5 bg-neutral-800 text-neutral-500 disabled:opacity-50 font-black text-[9px] uppercase tracking-wider rounded-lg transition-all cursor-not-allowed flex items-center gap-1"
                            >
                              🔒 Approuver (Désactivé)
                            </button>
                            <button 
                              disabled={true}
                              className="px-3 py-1.5 bg-neutral-900 border border-neutral-800 text-neutral-600 font-black text-[9px] uppercase tracking-wider rounded-lg transition-all cursor-not-allowed flex items-center gap-1"
                            >
                              🔒 Refuser (Désactivé)
                            </button>
                          </div>
