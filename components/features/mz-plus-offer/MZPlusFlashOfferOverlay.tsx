...
import {
  ...
  formatAmountForSentence,
} from "./MZPlusOnboardingGoalsModal.tsx";
...
const LiveActivityPulse = () => {
  const [activities, setActivities] = useState([
    {
      id: 1,
      text: "Gains en cours : +45,000 FCFA (RPA Protocol)",
      user: "Moussa.K",
      type: "earn",
    },
    {
      id: 2,
      text: "LICENCE ÉLITE ACTIVÉE",
      user: "Sarah_Elite",
      type: "upgrade",
    },
    {
      id: 3,
      text: "Extraction de commissions terminée",
      user: "Membre_#482",
      type: "success",
    },
    {
      id: 4,
      text: "Nouveau membre Premium : Amadou",
      user: "Amadou-CI",
      type: "upgrade",
    },
  ]);
  ...
  return (
    <div className="fixed top-24 left-6 z-[100] pointer-events-none hidden xl:block w-80">
      <div className="text-[8px] font-black tracking-[0.3em] text-purple-500 mb-4 animate-pulse">
        LIVE ACCESS STREAM // MZ+ NETWORK
      </div>
      <AnimatePresence mode="popLayout">
        {activities.map((act) => (
          ...
          >
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${act.type === "upgrade" ? "bg-purple-600 text-white" : "bg-emerald-600/20 text-emerald-500"}`}
            >
              {act.type === "upgrade" ? <Crown size={14} /> : <Zap size={14} />}
            </div>
            <div className="overflow-hidden">
              <p className="text-[9px] text-neutral-500 font-black uppercase tracking-widest truncate">
                {act.user}
              </p>
...
> = ({ profile, onUpgrade, onClose, isFullPage = false }) => {
  ...
  const [errorMessageChariow, setErrorMessageChariow] = useState<string | null>(
    null,
  );
  ...
  const trackClick = (elementId: string) => {
    ...
    metadataRef.current.clicks[elementId] =
      (metadataRef.current.clicks[elementId] || 0) + 1;
  };

  const trackFieldFilled = (fieldName: string, isFilled: boolean) => {
    if (!metadataRef.current.fields_filled)
      metadataRef.current.fields_filled = {};
    metadataRef.current.fields_filled[fieldName] = isFilled;
  };

  const trackError = (errorMessage: string) => {
    if (!metadataRef.current.errors_encountered)
      metadataRef.current.errors_encountered = [];
  ...
  useEffect(() => {
    ...
    const initTracking = async () => {
      try {
        ...
        if (!error && data) {
          ...
          console.log(
            "[Analytical Tracker] Session successfully started with ID :",
            data.id,
          );
        } else {
          console.warn(
            "[Analytical Tracker] Could not initialize database record:",
            error,
          );
        }
      } catch (err) {
    ...
    const pingInterval = setInterval(async () => {
      ...
      const elapsedSecs = Math.round(
        (Date.now() - trackingStartRef.current) / 1000,
      );
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    metadataRef.current.max_scroll_percent = Math.max(
      metadataRef.current.max_scroll_percent || 0,
      scrollPercent,
    );
    ...
    if (
      scrollPercent >= 10 &&
      !scrolledList.includes("onboarding_trajectory")
    ) {
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const hasVisited = localStorage.getItem(
        "mz_flash_offer_completed_first_view",
      );
      if (hasVisited === "true") {
        setIsReturning(true);
      } else {
  ...
  useEffect(() => {
    const checkQualifications = async () => {
      ...
      try {
        ...
        const sumDbMins =
          trackingData?.reduce(
            (acc: number, item: any) => acc + (item.total_minutes || 0),
            0,
          ) || 0;
        if (sumDbMins >= 6) {
          dbMinsPassed = true;
      ...
      const isQualified =
        isFirst || sessionPassed || isCumulativeSecsPassed || dbMinsPassed;
  ...
  useEffect(() => {
    ...
    profile?.id,
    profile?.store_preferences?.onboarding_goals,
    qualifiesForQuiz,
  ]);
  ...
  const handleChariowCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingProd) {
        ...
        const fallbackProd = {
          ...
          name: "MZ+ Premium",
          description:
            "Accès illimité aux fonctionnalités MZ+ Premium (Offre Flash)",
          price: config?.price_promo
            ? parseInt(String(config.price_promo))
            : 15000,
      ...
      console.error(
        "[Checkout Form] Erreur traitement association produit :",
        err,
      );
    }
    ...
    if (
      ...
      cleanPhone.length > dialCode.length
    ) {
      ...
      if (json.success) {
        ...
        if (referrerCode) {
          try {
            ...
            console.error(
              "[Checkout Form] Échec de création d'une commission pending:",
              err,
            );
          }
        }
        ...
        if (checkoutUrl) {
          ...
          const errStr =
            "Impossible de récupérer le lien de paiement de Chariow.";
        ...
        const errStr =
          json.message ||
          "Erreur de l'API lors de l'initialisation du paiement.";
        setErrorMessageChariow(errStr);
        trackError(errStr);
      }
    } catch (err) {
      const errStr =
        "Une erreur réseau ou serveur s'est produite lors du paiement.";
      setErrorMessageChariow(errStr);
      trackError(errStr);
    } finally {
  ...
  return (
    ...
    >
      ...
      <div
        className={`transition-all duration-700 w-full min-h-screen ${isShowingChariowForm ? "blur-2xl brightness-[0.25] saturate-50 scale-[0.98] pointer-events-none select-none" : ""}`}
      >
        <LiveActivityPulse />
        ...
        <nav className="fixed top-0 left-0 right-0 z-[100] p-6 flex justify-between items-center max-w-7xl mx-auto">
          ...
          >
            <div className="bg-purple-600 p-2 rounded-xl shadow-[0_0_30px_rgba(147,51,234,0.6)]">
              <Crown size={24} />
            </div>
            <span className="text-[10px] font-black uppercase tracking-[0.5em] hidden md:block">
              MZ+ ELITE INTERFACE // V2.4
            </span>
          </motion.div>
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-6 pt-40 pb-64 space-y-48">
                  ...
                  <div className="space-y-4 flex flex-col items-center justify-center text-center">
                    <div className="space-y-3 font-sans">
                      ...
                      <p className="text-base sm:text-lg leading-relaxed text-neutral-200">
                        ...
                        , tu es peut-être à un pas de l’objectif que tu veux
                        atteindre : générer{" "}
                        <span className="font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-300 to-emerald-400 drop-shadow-[0_0_12px_rgba(245,158,11,0.5)] animate-pulse inline-block uppercase tracking-wide px-2 py-0.5 rounded bg-yellow-500/5 border border-yellow-500/20 whitespace-nowrap">
                          {formatAmountForSentence(
                            profile?.store_preferences?.onboarding_goals
                              ?.amount_selected,
                            currency,
                      ...
                      <p className="text-xs sm:text-sm text-neutral-400 max-w-xs mx-auto leading-relaxed">
                        Obtiens l'accompagnement personnalisé qui va te
                        permettre d'atteindre ton objectif le plus rapidement
                        possible.
                      </p>
                    </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4 md:space-y-6">
                <p className="text-lg sm:text-xl md:text-4xl lg:text-5xl text-neutral-200 font-black max-w-5xl mx-auto leading-tight italic tracking-tighter px-4">
                  "Tu es motivé, tu veux réussir en ligne…{" "}
            ...
            >
              <div className="flex flex-col md:flex-row gap-4 items-center justify-center w-full max-w-4xl mx-auto">
                ...
                >
                  ...
                  <span className="relative z-10 group-hover:text-white transition-colors flex items-center justify-center gap-3">
                    OBTENIR MON ACCOMPAGNEMENT & MA RÉUSSITE{" "}
              ...
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-neutral-600 text-center">
                  Paiement ultra-sécurisé par cryptage AES-256
                </div>
              </div>
          ...
          <section className="max-w-4xl mx-auto">
            ...
            >
              <h2 className="text-3xl sm:text-4xl md:text-6xl font-black uppercase italic leading-none tracking-tighter text-center">
                L'ERREUR FATALE <br />{" "}
                <span className="text-neutral-600">DU NIVEAU STANDARD.</span>
              </h2>
              <div className="grid md:grid-cols-3 gap-6 md:gap-10">
                {[
                  {
                    t: "L'invisibilité algorithmique",
                    d: "Sans Premium, tes chances de voir les opportunités à 6 chiffres sont divisées par 10.",
                  },
                  {
                    t: "Le plafond de verre",
                    d: "Tu es limité à des miettes pendant que les membres Élite dévorent le marché.",
                  },
                  {
                    t: "L'isolement total",
                    d: "Avancer seul est le chemin le plus court vers l'abandon.",
                  },
                ].map((item, i) => (
                  <div
                    key={i}
                    className="flex flex-col items-center md:items-start gap-4 md:gap-6 group text-center md:text-left"
                  >
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-white/5 border border-white/10 rounded-xl md:rounded-[1.5rem] flex items-center justify-center shrink-0 group-hover:bg-red-600/10 group-hover:border-red-500/50 transition-all shadow-xl">
                      <AlertCircle
                        size={24}
                        className="text-red-500 md:w-7 md:h-7"
                      />
                    </div>
                    <div className="space-y-0.5 md:space-y-1">
                      <h4 className="text-lg md:text-xl font-black uppercase italic text-white/90">
                        {item.t}
                      </h4>
                      <p className="text-xs md:text-base text-neutral-500 font-medium leading-relaxed">
                        {item.d}
                      </p>
                    </div>
                  </div>
          ...
          <section
            id="proofs-section"
            className="space-y-8 md:space-y-12 scroll-mt-20"
          >
            ...
            <div className="text-center space-y-3 max-w-2xl mx-auto px-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-purple-500/10 border border-purple-500/20 rounded-full">
                ...
                <span className="text-[10px] font-bold tracking-widest text-purple-300 uppercase">
                  CECI VA CHANGER VOTRE VISION DES CHOSES
                </span>
              </div>
              ...
              <p className="text-neutral-400 text-xs sm:text-sm leading-relaxed">
                Ils étaient exactement là où vous êtes aujourd'hui : bloqués,
                impatients et pleins de doutes. Découvrez en un coup d'œil
                comment leur situation s'est débloquée.
              </p>
            </div>
            ...
            {(() => {
              const fallbackProofs = [
                {
                  ...
                  description:
                    "J’ai passé 10 jours en mode standard à attendre sans rien recevoir. J'ai franchi le pas pour activer Premium un vendredi soir. Le dimanche matin, premier virement de 40 000 FCFA sur mon compte !",
                  after_image_url:
                ...
                },
                {
                  ...
                  description:
                    "Tout le monde me disait que ça n'allait jamais marcher pour moi. J'ai quand même activé Premium en me disant que je n'avais rien à perdre. En 5 jours, j'ai cumulé et retiré 150 000 FCFA. Une immense fierté devant mes amis !",
                  after_image_url:
                ...
                },
                {
                  ...
                  description:
                    "Je voulais juste gagner un peu de quoi soulager mes fins de mois difficiles. Quand j'ai activé Premium, tout s'est accéléré tellement vite. J'ai fait 500 000 FCFA en deux semaines. Cela a changé la vie de toute ma maison.",
                  after_image_url:
                    "https://images.unsplash.com/photo-1559526324-4b87b5e36e44?auto=format&fit=crop&q=80&w=400",
              ...
              let displayedProofs =
                activeProofs.length > 0 ? activeProofs : fallbackProofs;
              ...
              const safeIndex =
                selectedProofIndex < displayedProofs.length
                  ? selectedProofIndex
                  : 0;
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      ...
                      <div className="md:col-span-5 flex flex-col justify-center space-y-4">
                        ...
                        {selectedProof.before_image_url ? (
                          <div className="grid grid-cols-2 gap-3 animate-fade-in">
                            {/* Before capture */}
                            <div className="relative aspect-[3/4] rounded-2xl overflow-hidden border border-white/5 bg-neutral-950 shadow-md group/before duration-300">
                              <img
                                src={getGDriveThumbnailUrl(
                                  selectedProof.before_image_url,
                                )}
                                alt={`Avant (${selectedProof.before_amount || "0"})`}
                              ...
                              />
                              <div className="absolute top-2 left-2 bg-neutral-900/95 backdrop-blur px-2.5 py-1 rounded-full border border-white/10 shadow-lg">
                                <span className="text-[8px] font-black tracking-wider text-neutral-400 uppercase font-sans">
                                  🔴 DEPART
                                </span>
                              </div>
                              <div className="absolute bottom-2 right-2 bg-black/85 backdrop-blur border border-white/10 px-2 py-1 rounded-lg text-[9px] font-mono text-neutral-400 font-bold leading-none">
                                {selectedProof.before_amount || "0 FCFA"}
                              </div>
                            </div>

                            {/* After capture */}
                            <div className="relative aspect-[3/4] rounded-2xl overflow-hidden border border-purple-500/20 bg-neutral-950 shadow-md group/after duration-300">
                              <img
                                src={getGDriveThumbnailUrl(
                                  selectedProof.after_image_url,
                                )}
                                alt={`Après (${selectedProof.after_amount})`}
                                className="w-full h-full object-cover cursor-pointer hover:scale-[1.02] transition-transform duration-500"
                                referrerPolicy="no-referrer"
                                onClick={() =>
                                  setFullscreenProof(selectedProof)
                                }
                              />
                              <div className="absolute top-2 left-2 bg-purple-600/90 backdrop-blur px-2.5 py-1 rounded-full border border-purple-400/30 shadow-lg">
                                <span className="text-[8px] font-black tracking-wider text-white uppercase font-sans">
                                  ✨ REUSSITE
                                </span>
                              </div>
                              <div className="absolute bottom-2 right-2 bg-purple-950/90 backdrop-blur border border-purple-500/30 px-2 py-1 rounded-lg text-[9px] font-mono text-purple-300 font-bold leading-none">
                                +{selectedProof.after_amount}
                              </div>
                            </div>
                          </div>
                        ) : (
                          <div className="relative aspect-[4/3] rounded-2xl overflow-hidden border border-purple-500/20 bg-neutral-950 shadow-2xl group/img">
                            {selectedProof.after_image_url ? (
                              ...
                              <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                                <Sparkles
                                  size={28}
                                  className="text-purple-400 animate-pulse mb-2"
                                />
                            ...
                            {/* Persistent Intuitive Click Indicator */}
                            <div className="absolute bottom-3 right-3 bg-black/80 backdrop-blur-md px-2.5 py-1.5 rounded-xl border border-white/10 flex items-center gap-2 shadow-2xl pointer-events-none">
                        ...
                        <div className="space-y-4">
                          <div className="space-y-1">
                            ...
                            <h3 className="text-lg sm:text-2xl font-black text-white uppercase tracking-tight font-sans">
                              ...
                              à{" "}
                              <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-300 via-pink-400 to-yellow-400">
                                +{selectedProof.after_amount}
                              </span>{" "}
                              obtenu {selectedProof.time_frame.toLowerCase()} !
                            </h3>
                          ...
                          <div className="bg-white/[0.01] border border-white/5 rounded-2xl p-4 space-y-4">
                            ...
                            <div className="relative flex items-center justify-between pt-1 px-1">
                              ...
                              <div className="flex flex-col items-center space-y-1.5">
                                ...
                                <span className="text-[7.5px] font-black text-neutral-600 uppercase tracking-wider">
                                  Compte Bloqué
                                </span>
                              </div>
                              ...
                              <div className="flex flex-col items-center space-y-1.5">
                                ...
                                <span className="text-[7.5px] font-black text-purple-400 uppercase tracking-wider">
                                  Accompagnement
                                </span>
                              </div>
                              ...
                              <div className="flex flex-col items-center space-y-1.5">
                                ...
                                <span className="text-[7.5px] font-black text-amber-400 uppercase tracking-wider">
                                  +{selectedProof.after_amount}
                                </span>
                              </div>
                            </div>
            ...
            <div className="max-w-4xl mx-auto space-y-8 md:space-y-10 px-4 md:px-8 relative z-10">
              <div className="flex justify-center">
                ...
                >
                  <Crown
                    size={32}
                    className="text-white md:w-[50px] md:h-[50px]"
                  />
                </motion.div>
              </div>
              ...
              <h3 className="text-4xl sm:text-5xl md:text-9xl font-black uppercase tracking-tighter italic leading-none break-words">
                DERNIER <br />
                <span className="text-purple-500 italic drop-shadow-[0_0_30px_rgba(168,85,247,0.5)]">
                  APPEL.
                </span>
              </h3>

              <p className="text-lg md:text-3xl font-bold text-neutral-300 leading-tight italic px-2">
                "Beaucoup regardent cette page. Très peu osent vraiment évoluer.{" "}
                <br className="hidden md:block" />
                <span className="text-white underline decoration-purple-600 underline-offset-4 md:underline-offset-8">
                  Lequel des deux es-tu aujourd'hui ?
                </span>
                "
              </p>
              ...
              <div className="pt-6 md:pt-10 space-y-8 md:space-y-12">
                <div className="flex flex-col items-center gap-4 md:gap-6">
                  <div className="flex flex-wrap items-center justify-center gap-4 md:gap-6">
                    <span className="text-neutral-700 line-through text-2xl md:text-4xl font-mono italic">
                      {config.price_normal} FCFA
                    </span>
                    <span className="text-5xl sm:text-6xl md:text-[9rem] font-mono font-black italic text-white tracking-tighter drop-shadow-[0_0_40px_rgba(255,255,255,0.1)] leading-none">
                      {config.price_promo}{" "}
                      <span className="text-xl md:text-2xl text-purple-600 italic uppercase">
                        FCFA
                      </span>
                    </span>
                  </div>
                ...
                >
                  ...
                  <Rocket
                    size={32}
                    className="md:w-[40px] md:h-[40px] group-hover:translate-x-2 group-hover:-translate-y-2 transition-transform"
                  />
                </button>
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 md:gap-8 opacity-40 text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-neutral-500">
                  <span className="flex items-center gap-2 md:gap-3">
                    <BadgeCheck size={12} className="md:w-[14px] md:h-[14px]" />{" "}
                    Paiement unique
                  </span>
                  <span className="flex items-center gap-2 md:gap-3">
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-[10px] text-neutral-400 font-bold uppercase tracking-widest font-mono">
                  ⏱️ Atteint en seulement{" "}
                  {fullscreenProof.time_frame || "2 jours"}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 max-h-[60vh] md:max-h-none overflow-y-auto pr-1">
                ...
                <div className="md:col-span-6 space-y-3">
                  <p className="text-[10px] font-black tracking-widest text-purple-400 uppercase leading-none">
                    📸 DOCUMENT DE REUSSITE VERIFIE
                  </p>
                  
                  {fullscreenProof.before_image_url ? (
                    <div className="grid grid-cols-2 gap-3">
                      {/* Before (Avant) Image */}
                      <div className="relative aspect-[3/4] max-h-[420px] rounded-2xl overflow-hidden border border-white/10 bg-neutral-950 flex items-center justify-center">
                        <img
                          src={getGDriveThumbnailUrl(fullscreenProof.before_image_url)}
                          alt="Preuve avant"
                          className="w-full h-full object-cover"
                          referrerPolicy="no-referrer"
                        />
                        <div className="absolute top-2 left-2 bg-neutral-900/95 backdrop-blur px-2 py-0.5 rounded-full border border-white/10 text-[7px] text-neutral-400 font-bold uppercase tracking-wider">
                          🔴 DEPART: {fullscreenProof.before_amount || "0 FCFA"}
                        </div>
                      </div>

                      {/* After (Après) Image */}
                      <div className="relative aspect-[3/4] max-h-[420px] rounded-2xl overflow-hidden border border-purple-500/30 bg-neutral-950 flex items-center justify-center">
                        <img
                          ...
                          referrerPolicy="no-referrer"
                        />
                        <div className="absolute top-2 left-2 bg-purple-600 px-2 py-0.5 rounded-full text-[7px] font-black font-sans text-white uppercase tracking-wider flex items-center gap-0.5 shadow-md">
                          <span>✨ REUSSITE</span>
                        </div>
                        <div className="absolute bottom-2 right-2 bg-black/90 backdrop-blur border border-purple-500/30 px-2 py-1 rounded-lg text-[9px] font-mono text-white font-black flex items-center gap-1">
                    ...
                    <div className="relative aspect-[3/4] max-h-[420px] rounded-2xl overflow-hidden border border-purple-500/30 bg-neutral-950 shadow-[0_0_40px_rgba(168,85,247,0.15)] flex items-center justify-center">
                      {fullscreenProof.after_image_url ? (
                        ...
                        <div className="text-center p-6">
                          <Crown
                            size={48}
                            className="text-purple-500/40 mx-auto mb-3 animate-pulse"
                          />
                          <span className="text-xs font-bold text-purple-400 uppercase tracking-widest">
                            Reçu validé et crypté
                          </span>
                        </div>
                      )}
                ...
                <div className="md:col-span-6 flex flex-col justify-between space-y-5">
                  <div className="space-y-4">
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 sm:p-5 space-y-4">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[9.5px] font-black text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-yellow-400">
                          STATUS: RECHARGÉ 🏆
                        </span>
                      </div>
                    ...
                    <div className="bg-neutral-900/40 border border-white/5 rounded-2xl p-4 space-y-2">
                      <p className="text-[8.5px] font-black tracking-widest text-neutral-500 uppercase">
                        IDENTITE EXPLOITANT
                      </p>
                      <div className="flex items-center gap-3">
                        <span className="w-10 h-10 rounded-full bg-purple-500/20 text-purple-300 font-extrabold flex items-center justify-center border border-purple-500/10 text-sm">
                          {fullscreenProof.name
                            ? fullscreenProof.name[0].toUpperCase()
                            : "M"}
                        </span>
                        <div>
                          ...
                          <p className="text-[10px] text-neutral-400 font-bold">
                            {fullscreenProof.country_flag} •{" "}
                            {fullscreenProof.short_tag}
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-purple-950/10 border border-purple-500/10 rounded-2xl space-y-1">
                      <p className="text-[8.5px] font-black uppercase tracking-widest text-purple-400">
                        📝 ANALYSE DE LA REUSSITE
                      </p>
                      <p className="text-xs sm:text-sm text-neutral-300 italic leading-relaxed">
                        "
                        {fullscreenProof.description ||
                          "Après des semaines bloqué sans résultats en mode standard, la mise à niveau Elite Premium a permis de libérer tout le potentiel. L'accélération automatique s'est montrée d'une efficacité redoutable."}
                        "
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t border-white/5 flex flex-col sm:flex-row gap-4 items-center justify-between">
                    <div className="text-center sm:text-left">
                      <p className="text-[8px] font-black uppercase text-neutral-500 tracking-wider font-mono">
                        Rejoignez l'élite
                      </p>
                      <p className="text-lg font-black font-mono text-white">
                        Tarif Promotionnel :{" "}
                        <span className="text-purple-400">
                          {config.price_promo} FCFA
                        </span>
                      </p>
                    </div>
                    <button
            ...
            >
              ...
              <div className="space-y-4 relative z-10 text-left">
                <div className="flex items-center gap-2.5">
                  <div className="p-2 bg-gradient-to-br from-purple-600 to-indigo-700 rounded-lg text-white shadow-lg shadow-purple-500/15">
                    <Crown size={18} className="animate-pulse" />
                  </div>
                  <div>
                    <h3 className="text-sm sm:text-base font-black uppercase tracking-tight text-white font-sans text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-white leading-tight">
                      Dernier pas vers ta réussite
                    </h3>
                    <p className="text-[8px] uppercase font-bold tracking-wider text-yellow-500 font-sans leading-none">
                      Cercle Élite MZ+ Premium
                    </p>
                  </div>
                </div>
                ...
                <div className="p-3 sm:p-4 bg-gradient-to-r from-purple-950/40 to-neutral-900 border border-purple-500/25 rounded-xl flex justify-between items-center font-mono relative overflow-hidden">
                  ...
                  <div>
                    <span className="text-[8px] text-purple-400 font-black uppercase tracking-widest block font-sans">
                      TARIF EXCLUSIF UNIQUE
                    </span>
                    <span className="text-[9px] text-neutral-500 font-sans">
                      Aucun abonnement récurrent
                    </span>
                  </div>
                  <span className="text-lg sm:text-xl font-black text-white drop-shadow-[0_0_10px_rgba(168,85,247,0.3)]">
                    {config.price_promo} FCFA
                  </span>
                </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-0.5">
                      <label className="text-[8px] text-neutral-500 font-bold uppercase tracking-wider block font-sans">
                        Prénom
                      </label>
                      <input
                        required
                      ...
                      <div className="flex gap-1.5">
                        ...
                        >
                          {countriesSorted.map((c) => (
                            ...
                            >
                              {getFlagEmoji(c.code)} {c.name} (+{c.dial}) (
                              {c.code})
                            </option>
                          ))}
                        </select>
                  ...
                  <div className="pt-1.5 flex flex-col gap-1.5">
                    ...
                    >
                      {isSubmittingChariow ? (
                        <>
                          <Loader2 size={12} className="animate-spin" /> Liaison
                          sécurisée...
                        </>
                      ) : (
                        <>
                          OUI, JE COMMENCE À ENCAISSER MES COMMISSIONS PREMIUM
                          DÈS CE SOIR ! 💸🚀 <ChevronRight size={14} />
                        </>
                      )}
                    </button>
