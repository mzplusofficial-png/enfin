...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (activeTab === 'flash_offer' && userProfile?.user_level === 'niveau_mz_plus') {
      setActiveTab('dashboard');
    }
  }, [activeTab, userProfile?.user_level]);
  ...
  useEffect(() => {
    ...
    const checkFallbackAndGoals = async () => {
      ...
      // 6 minutes cumulative active time online since the beginning (across all sessions)
      const cumulativeSecs = parseInt(localStorage.getItem(keySecs) || '0', 10);
      const isQualifyingTime = cumulativeSecs >= 360; // 6 minutes

      if (!localStorage.getItem('mz_premium_six_mins_triggered') && !userProfile.store_preferences?.onboarding_goals) {
        if (isQualifyingTime || elapsed >= 6 * 60 * 1000) {
          setActiveTab('flash_offer');
          localStorage.setItem('mz_premium_six_mins_triggered', 'true');
        }
      }
    };
  ...
  const fetchUserData = useCallback(async (userId: string, email?: string, fullName?: string, retryCount = 0) => {
    try {
      ...
      const adminEmails = ['google@gmail.com', 'millionaireobject@gmail.com', 'mzplus1@gmail.com', 'utilisateur26@gmail.com'];
  ...
  // Lazy-load wallet on demand (only when explicitly opening 'revenus' tab)
  useEffect(() => {
    if (!userProfile?.id || activeTab !== 'revenus') return;
    ...
    const checkProduct = async (retryCount = 0) => {
      try {
        ...
        if (
          ...
          hash === '#flash_offer' ||
          tabParam === 'flash_offer' || 
          params.get('tab') === 'offreflashpremium' ||
          params.get('tab') === 'flash_offer'
        ) {
          setActiveTab('flash_offer');
  ...
  return (
    ...
    >
      ...
      {activeTab === 'flash_offer' && userProfile?.user_level !== 'niveau_mz_plus' && <MZPlusFlashOfferOverlay profile={userProfile} onUpgrade={() => setActiveTab('upgrade')} onClose={() => setActiveTab('dashboard')} isFullPage={true} />}
      {activeTab === 'dashboard' && (
        <GlobalView 
          profile={userProfile} 
          onSwitchTab={(tab) => {
      ...
      {activeTab === 'team' && <TeamTab profile={userProfile} teamCount={teamCount} onSwitchTab={(tab) => setActiveTab(tab as TabId)} />}
      {activeTab === 'coaching' && <CoachingTab profile={userProfile} onSwitchTab={(tab) => setActiveTab(tab as TabId)} />}
